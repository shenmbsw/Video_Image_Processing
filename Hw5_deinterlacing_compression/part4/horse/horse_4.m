...
title('MC prediction error for N = 32,Dmax = 1')
subplot(3,2,2);
imhist(uint8(predict_error_1(:,:,2)+128));
title('entropy 2.3525');
subplot(3,2,3);
imshow(uint8(predict_error_2(:,:,2)+128));
title('MC prediction error N = 32,Dmax = 5')
subplot(3,2,4);
imhist(uint8(predict_error_2(:,:,2)+128));
title('entropy 2.4252');
subplot(3,2,5);
imshow(uint8(predict_error_3(:,:,2)+128));
title('MC prediction error N = 32,Dmax = 10')
subplot(3,2,6);
imhist(uint8(predict_error_3(:,:,2)+128));
title('entropy 2.54814');
